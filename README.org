...
3. =!<default-package>=
   ...
   A default configuration set includes packages and settings enabled by
   default. It pulls the packages into the world dependency, but can be
   deactivated.

   The configuration will be inserted right before the machine set, hence it
   overrides both weak sets and general sets, but can be overridden by the
   machine set.

   By convention, it groups settings around orthogonal features (X, wayland,
   pipewire, etc.) which is enabled by default, and could be disabled in the
   machine set file.

4. =?<order>-<default-setting>=

   A weak configuration set defines package options or/and suggestions, without
   pulling packages as the world dependency.

   The configuration will be *prepended* to the paludis configurations.

   By convention, it groups (overridable) global defaults that will be always
   enabled.
